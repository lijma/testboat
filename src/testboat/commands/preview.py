"""testboat preview — serve reports locally and optionally export PDF."""
...
def _pid_path(testboat_root: Path) -> Path:
    return testboat_root / ".testboat" / PID_FILE


def _kill_existing(testboat_root: Path) -> bool:
    """Kill any existing preview process. Returns True if a process was killed."""
    pid_file = _pid_path(testboat_root)
...
def _write_pid(testboat_root: Path, pid: int) -> None:
    _pid_path(testboat_root).write_text(str(pid))
...
def serve_reports(
    ...
    Returns (port, url).
    """
    from testboat.commands.report import generate_index
    _kill_existing(testboat_root)
    generate_index(testboat_root)

    serve_dir = testboat_root / ".testboat"
    ...
    _write_pid(testboat_root, os.getpid())
    ...
    def _watchdog():
        stop.wait()
        server.shutdown()
        _pid_path(testboat_root).unlink(missing_ok=True)
