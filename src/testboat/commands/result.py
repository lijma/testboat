"""testboat result — execution results + global execution matrix."""
...
def _results_dir(testboat_root: Path) -> Path:
    return active_dir(testboat_root) / RESULTS_DIR


def _result_path(testboat_root: Path, res_id: str) -> Path:
    return _results_dir(testboat_root) / f"{res_id}.yaml"


def _matrix_path(testboat_root: Path) -> Path:
    return active_dir(testboat_root) / MATRIX_FILE


def _next_result_id(testboat_root: Path) -> str:
    existing = sorted(_results_dir(testboat_root).glob("RES-*.yaml"))
    if not existing:
        return "RES-001"
...
def _load_matrix(testboat_root: Path) -> dict[str, Any]:
    path = _matrix_path(testboat_root)
    if not path.exists():
        return {}
    return yaml.safe_load(path.read_text(encoding="utf-8")) or {}


def _save_matrix(testboat_root: Path, data: dict[str, Any]) -> None:
    path = _matrix_path(testboat_root)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(yaml.dump(data, default_flow_style=False, allow_unicode=True), encoding="utf-8")


def _update_matrix(testboat_root: Path, tc_id: str, res_id: str,
    ...
    matrix[tc_id] = entry
    _save_matrix(testboat_root, matrix)
...
def record_result(
    ...
    _results_dir(testboat_root).mkdir(parents=True, exist_ok=True)
    res_id = _next_result_id(testboat_root)
    ...
    path = _result_path(testboat_root, res_id)
    path.write_text(yaml.dump(data, default_flow_style=False, allow_unicode=True), encoding="utf-8")

    _update_matrix(testboat_root, tc_id, res_id, status, execution_type, now)
    return path


def list_results(testboat_root: Path, tc_id: str | None = None) -> list[dict[str, Any]]:
    """Return list of results, optionally filtered by tc_id."""
    results_dir = _results_dir(testboat_root)
...
def show_result(testboat_root: Path, res_id: str) -> dict[str, Any]:
...
def get_matrix(testboat_root: Path, tc_id: str | None = None) -> dict[str, Any]:
