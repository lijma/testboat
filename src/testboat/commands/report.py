...
<script type="application/json" id="tc-json">{{ tc_json }}</script>
<script>
var TC_DATA = JSON.parse(document.getElementById('tc-json').textContent);
...
def _build_tc_json(data: dict) -> str:
    ...
    # ensure_ascii=True: all non-ASCII chars become \uXXXX — eliminates
    # U+2028/U+2029 (JS line-terminator) and other Unicode edge cases.
    # Replace </ with <\/ : prevents the HTML parser from misreading
    # </script> inside a JSON string value and closing the <script> element.
