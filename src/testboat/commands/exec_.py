"""testboat exec — run automation scripts and auto-record results."""
...
from testboat.commands.plan import get_run_command, list_plans
...
def run_case(
    ...
    """
    tool, cmd = get_run_command(testboat_root, tc_id)
    if _runner is None:
...
def run_sprint(
    ...
    """
    plans = list_plans(testboat_root, execution_type=None, status="approved")
    ...
    for plan in plans:
        ...
        tc_id = plan["tc_id"]
        try:
            status, res_id = run_case(testboat_root, tc_id, by=by, _runner=_runner)
