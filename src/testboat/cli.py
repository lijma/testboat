...
from testboat.commands.enable import AGENTS, _CONTENT_MAP, _resolve, enable_agent
from testboat.commands.init import DRAFT_SUBDIRS, init_workspace
...
from testboat.commands.preview import export_pdf, serve_reports
from testboat.commands.active import get_active_version
from testboat.commands.version import create_version, get_current_active, list_versions, show_version, switch_version
from testboat.commands.report import generate_closure, generate_sprint, generate_strategy
from testboat.commands.strategy import create_strategy, validate_strategy
from testboat.commands.tag import TAG_KINDS, add_tag, list_tags
from testboat.commands.validate import run_validate
...
def init(
    ...
    """Initialise (or refresh) .testboat/ runtime directory in the workspace."""
    target = _workspace(workspace)
    testboat_path = init_workspace(target)
    typer.echo(f"Initialized testboat workspace at {testboat_path}")
    typer.echo("  draft/  ← current working version")
    for subdir in DRAFT_SUBDIRS:
        typer.echo(f"    {subdir}/")
    typer.echo("    testboat.yaml")
...
def enable(
    ...
    """Create (or overwrite) testboat rules + skill files for the specified agent."""
    if agent == "list":
    ...
    typer.echo(f"Enabled testboat for '{label}':")
    for path in created:
...
def strategy_create(
    ...
    typer.echo("  Edit the file, then run: testboat strategy validate")
...
def case_add(
    ...
    typer.echo(f"  → {path.relative_to(target)}")
    typer.echo("  Next: ask AI to fill in steps, then run: testboat case validate " + tc_id)
...
def report_sprint(
    ...
    typer.echo(f"Generated → {path.relative_to(target)}")
    typer.echo("  Run `testboat preview` to open in browser.")
...
def version_switch(
    ...
    """Switch the active version (updates .testboat/.active)."""
    target = _workspace(workspace)
    try:
    ...
    typer.echo("  All testboat commands now operate on this version's directory.")
