"""ftest version — snapshot and manage named test artifact versions."""
...
from ftest.commands.active import DEFAULT_VERSION, get_active_version, set_active_version
...
def _ftest_root_dir(ftest_root: Path) -> Path:
    return ftest_root / ".ftest"


def _draft_path(ftest_root: Path) -> Path:
    return _ftest_root_dir(ftest_root) / DRAFT_DIR


def _version_path(ftest_root: Path, version: str) -> Path:
    return _ftest_root_dir(ftest_root) / version


def _list_versions(ftest_root: Path) -> list[str]:
    """Return sorted list of existing version names (excludes draft)."""
    ftest_dir = _ftest_root_dir(ftest_root)
    if not ftest_dir.exists():
        return []
    return sorted(
        d.name for d in ftest_dir.iterdir()
...
def create_version(ftest_root: Path, version: str, base: str | None = None) -> Path:
    """Create a new named version.

    If *base* is None: copy draft → .ftest/<version>/
    If *base* is set:  copy .ftest/<base>/ → .ftest/<version>/
    ...
    dest = _version_path(ftest_root, version)
    if dest.exists():
        raise FileExistsError(f"Version '{version}' already exists.")

    if base is None:
        src = _draft_path(ftest_root)
        if not src.exists():
            raise FileNotFoundError("No draft found. Run `ftest init` first.")
    else:
        src = _version_path(ftest_root, base)
        if not src.exists():
...
def list_versions(ftest_root: Path) -> list[dict[str, Any]]:
    """Return list of version info dicts, sorted by name."""
    result = []
    for name in _list_versions(ftest_root):
        vdir = _version_path(ftest_root, name)
...
def switch_version(ftest_root: Path, version: str) -> str:
    ...
    if version != DEFAULT_VERSION:
        vdir = _version_path(ftest_root, version)
        if not vdir.exists():
            raise FileNotFoundError(f"Version '{version}' not found.")
    set_active_version(ftest_root, version)
    return version


def get_current_active(ftest_root: Path) -> str:
    """Return the currently active version name."""
    return get_active_version(ftest_root)


def show_version(ftest_root: Path, version: str) -> dict[str, Any]:
