"""ftest report — generate HTML test reports from .ftest/draft/ artifacts."""
...
def _load_strategy(ftest_root: Path) -> dict[str, Any]:
    path = active_dir(ftest_root) / "strategy.yaml"
    if not path.exists():
        return {}
    return yaml.safe_load(path.read_text(encoding="utf-8")) or {}


def _load_cases(ftest_root: Path) -> list[dict[str, Any]]:
    cases_dir = active_dir(ftest_root) / "cases"
...
def _load_results(ftest_root: Path) -> list[dict[str, Any]]:
    results_dir = active_dir(ftest_root) / "executions" / "results"
...
def _load_matrix(ftest_root: Path) -> dict[str, Any]:
    path = active_dir(ftest_root) / "executions" / "execution-matrix.yaml"
    if not path.exists():
        return {}
    return yaml.safe_load(path.read_text(encoding="utf-8")) or {}


def _load_bugs(ftest_root: Path) -> list[dict[str, Any]]:
    bugs_dir = active_dir(ftest_root) / "bugs"
...
def _reports_dir(ftest_root: Path) -> Path:
    return active_dir(ftest_root) / REPORTS_DIR
...
<footer>Generated by ftest · {{ ts }}</footer>
</div></body></html>"""
...
def _write_report(ftest_root: Path, filename: str, html: str) -> Path:
    out = _reports_dir(ftest_root)
...
def generate_strategy(ftest_root: Path) -> Path:
    """Render strategy.yaml → strategy.html. Returns output path."""
    strategy = _load_strategy(ftest_root)
    ...
    return _write_report(ftest_root, f"strategy-{release}.html", html)
...
def _compute_sprint_data(
    ...
    strategy = _load_strategy(ftest_root)
    cases = _load_cases(ftest_root)
    matrix = _load_matrix(ftest_root)
    bugs = _load_bugs(ftest_root)
...
def generate_sprint(ftest_root: Path) -> Path:
    """Render sprint test report. Returns output path."""
    data = _compute_sprint_data(ftest_root)
    html = _render(_SPRINT_TMPL, **data)
    return _write_report(ftest_root, f"sprint-{data['release']}.html", html)


def generate_closure(ftest_root: Path, summary: str = "") -> Path:
    ...
    return _write_report(ftest_root, f"closure-{data['release']}.html", html)
...
<title>ftest — Test Dashboard</title>
<style>{{ css }}</style>
</head>
<body>
<div class="hero">
  <div class="hero-inner">
    <div class="hero-badge">🧪 ftest — Manage Tests Like Code</div>
...
<div class="section" style="margin-top:32px">
  ...
  <div class="grid">
    ...
    {% endfor %}
    {% if not versions %}
    <div class="empty">No versions found. Run <code>ftest init</code> first.</div>
    {% endif %}
  </div>
</div>

<footer>Generated by <strong>ftest</strong> · {{ ts }}</footer>
</body>
</html>"""
...
<html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>ftest Reports — {{ release }}</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;
     background:#0f1117;color:#e2e8f0;min-height:100vh}
...
<div class="section">
  ...
  <div class="report-grid">
    ...
    <div style="color:#64748b;font-size:14px;padding:24px">
      No reports generated yet. Run <code>ftest report strategy</code>,
      <code>ftest report sprint</code>, or <code>ftest report closure</code>.
    </div>
    {% endif %}
...
def _per_version_report_index(ftest_root: Path, version_name: str,
...
def generate_index(ftest_root: Path) -> Path:
    ...
    from ftest.commands.version import _list_versions, _read_meta, _version_path
    for vname in _list_versions(ftest_root):
        vdir = _version_path(ftest_root, vname)
        meta = _read_meta(vdir)
        stats = _version_stats(vdir)
        reports_dir = vdir / "reports"
        _per_version_report_index(ftest_root, vname, vdir, reports_dir)
        all_versions.append(dict(
            name=vname,
            base=meta.get("base"),
