"""ftest case — test case CRUD + state machine."""
...
from ftest.commands.tag import tag_exists
...
def _cases_dir(ftest_root: Path) -> Path:
    return active_dir(ftest_root) /  CASES_DIR


def _case_path(ftest_root: Path, tc_id: str) -> Path:
    return _cases_dir(ftest_root) / f"{tc_id}.yaml"


def _next_id(ftest_root: Path) -> str:
    existing = sorted(_cases_dir(ftest_root).glob("TC-*.yaml"))
    if not existing:
        return "TC-001"
...
def _load_case(ftest_root: Path, tc_id: str) -> dict[str, Any]:
    path = _case_path(ftest_root, tc_id)
    if not path.exists():
        raise FileNotFoundError(f"{tc_id} not found in cases/")
    return yaml.safe_load(path.read_text(encoding="utf-8"))


def _save_case(ftest_root: Path, data: dict[str, Any]) -> Path:
    path = _case_path(ftest_root, data["id"])
...
def add_case(
    ...
    for kind, value in [("sprint", sprint), ("type", type_), ("module", module)]:
        if value and not tag_exists(ftest_root, kind, value):
            raise ValueError(
                f"Tag {kind}='{value}' does not exist. "
                f"Run `ftest tag add {kind} {value}` first."
            )

    tc_id = _next_id(ftest_root)
    data: dict[str, Any] = {
        "id": tc_id,
        "title": title,
    ...
    return _save_case(ftest_root, data)
...
def list_cases(
    ...
    cases_dir = _cases_dir(ftest_root)
...
def show_case(ftest_root: Path, tc_id: str) -> dict[str, Any]:
    """Return case data. Raises FileNotFoundError if not found."""
    return _load_case(ftest_root, tc_id)


def set_status(ftest_root: Path, tc_id: str, new_status: str) -> None:
    """Transition tc_id to new_status.

    Raises ValueError for invalid transitions.
    Raises FileNotFoundError if tc_id not found.
    """
    data = _load_case(ftest_root, tc_id)
...
def validate_cases_batch(
    ...
    """
    cases = list_cases(ftest_root, sprint=sprint, type_=type_,
                       module=module, status=status)
    return {c["id"]: validate_case(ftest_root, c["id"]) for c in cases}


def validate_case(ftest_root: Path, tc_id: str) -> list[str]:
