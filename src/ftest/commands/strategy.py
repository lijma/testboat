"""ftest strategy — create and validate test strategy YAML."""
...
def _strategy_path(ftest_root: Path) -> Path:
    return active_dir(ftest_root) /  STRATEGY_FILE


def create_strategy(ftest_root: Path) -> Path:
    """Write strategy.yaml template under .ftest/draft/strategy/.
...
def validate_strategy(ftest_root: Path) -> list[str]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"strategy.yaml not found at {path}. Run `ftest strategy create` first."
        )
