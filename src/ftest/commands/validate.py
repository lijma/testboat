"""ftest validate — pre-report health check across all artifacts."""
...
from ftest.commands.bug import list_bugs
from ftest.commands.case import list_cases, validate_case
from ftest.commands.plan import list_plans, show_plan
from ftest.commands.result import get_matrix
from ftest.commands.strategy import STRATEGY_FILE, validate_strategy
...
def _check_format(ftest_root: Path) -> CheckResult:
    """Validate strategy.yaml and all TC yamls against their schemas."""
    details: list[str] = []
    passed = True

    # strategy.yaml
    strategy_path = active_dir(ftest_root) / STRATEGY_FILE
    if not strategy_path.exists():
        details.append("✗ strategy.yaml not found — run `ftest strategy create`")
        passed = False
    else:
        errors = validate_strategy(ftest_root)
        if errors:
            passed = False
            for e in errors:
    ...
    if not cases:
        ...
        tc_errors: list[str] = []
        for c in cases:
            errs = validate_case(ftest_root, c["id"])
            for e in errs:
...
def _check_requirements_coverage(ftest_root: Path) -> CheckResult:
...
def _check_execution_completeness(ftest_root: Path) -> CheckResult:
    """Check all non-draft TCs have at least one execution result."""
    details: list[str] = []
    passed = True

    cases = list_cases(ftest_root)
    matrix = get_matrix(ftest_root)
...
def _check_exit_criteria(ftest_root: Path) -> CheckResult:
...
def run_validate(ftest_root: Path) -> ValidateReport:
    """Run all 4 validation checks and return a ValidateReport."""
    report = ValidateReport()
    report.checks.append(_check_format(ftest_root))
    report.checks.append(_check_requirements_coverage(ftest_root))
    report.checks.append(_check_execution_completeness(ftest_root))
    report.checks.append(_check_exit_criteria(ftest_root))
    return report
