"""ftest bug — bug / defect lifecycle management."""
...
def _bugs_dir(ftest_root: Path) -> Path:
    return active_dir(ftest_root) /  BUGS_DIR


def _bug_path(ftest_root: Path, bug_id: str) -> Path:
    return _bugs_dir(ftest_root) / f"{bug_id}.yaml"


def _next_id(ftest_root: Path) -> str:
    existing = sorted(_bugs_dir(ftest_root).glob("BUG-*.yaml"))
    if not existing:
        return "BUG-001"
...
def _load_bug(ftest_root: Path, bug_id: str) -> dict[str, Any]:
    path = _bug_path(ftest_root, bug_id)
    if not path.exists():
        raise FileNotFoundError(f"{bug_id} not found in bugs/")
    return yaml.safe_load(path.read_text(encoding="utf-8"))


def _save_bug(ftest_root: Path, data: dict[str, Any]) -> Path:
    path = _bug_path(ftest_root, data["id"])
...
def add_bug(
    ...
    return _save_bug(ftest_root, data)
...
def show_bug(ftest_root: Path, bug_id: str) -> dict[str, Any]:
    """Return bug data. Raises FileNotFoundError if not found."""
    return _load_bug(ftest_root, bug_id)


def set_bug_status(ftest_root: Path, bug_id: str, new_status: str) -> None:
    ...
    data = _load_bug(ftest_root, bug_id)
