"""ftest plan — execution plan per test case."""
...
def _plans_dir(ftest_root: Path) -> Path:
    return active_dir(ftest_root) / PLANS_DIR


def _plan_path(ftest_root: Path, tc_id: str) -> Path:
    return _plans_dir(ftest_root) / f"{tc_id}-plan.yaml"


def _automate_root(ftest_root: Path) -> Path:
    return active_dir(ftest_root) / "executions" / "automate"


def _automate_path(ftest_root: Path, tc_id: str) -> Path:
    return _automate_root(ftest_root) / tc_id


def _load_plan(ftest_root: Path, tc_id: str) -> dict[str, Any]:
    path = _plan_path(ftest_root, tc_id)
    if not path.exists():
        raise FileNotFoundError(f"Plan for {tc_id} not found. Run `ftest plan create {tc_id}` first.")
    return yaml.safe_load(path.read_text(encoding="utf-8"))


def _save_plan(ftest_root: Path, data: dict[str, Any]) -> Path:
    path = _plan_path(ftest_root, data["tc_id"])
...
def create_plan(
    ...
    automation_path: str | None = None
    if resolved_tool:
        automate_dir = _automate_path(ftest_root, tc_id)
        automation_path = str(automate_dir.relative_to(ftest_root))
    ...
    return _save_plan(ftest_root, data)
...
def list_plans(
    ...
    plans_dir = _plans_dir(ftest_root)
...
def show_plan(ftest_root: Path, tc_id: str) -> dict[str, Any]:
    """Return plan data. Raises FileNotFoundError if not found."""
    return _load_plan(ftest_root, tc_id)


def set_plan_status(ftest_root: Path, tc_id: str, new_status: str) -> None:
    ...
    data = _load_plan(ftest_root, tc_id)
    data["status"] = new_status
    _save_plan(ftest_root, data)
...
def register_automation(
    ...
    *script_path* is relative to ftest_root (e.g. .ftest/draft/executions/automate/pytest/tests/test_TC001.py).
...
def get_run_command(ftest_root: Path, tc_id: str) -> tuple[str, str]:
    ...
    return tool, cmd_template.format(path=str(ftest_root / path))
